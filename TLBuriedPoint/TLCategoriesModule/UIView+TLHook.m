...
#define TL_FINGERPRINT_VERSION 1

@implementation UIView (TLHook)

- (int)tl_fingerprintVersion {
    return TL_FINGERPRINT_VERSION;
}
...
- (UIImage *)tl_snapshotForBlur {
    UIImage *image = [self tl_snapshotImage];
    // hack, helps with colors when blurring
    NSData *imageData = UIImageJPEGRepresentation(image, 1); // convert to jpeg
    return [UIImage imageWithData:imageData];
}

- (NSArray *)tl_targetActions {
...
// Set by a userDefinedRuntimeAttr in the SPTagNibs.rb script
- (void)setSpecterViewId:(id)object
{
    objc_setAssociatedObject(self, @selector(specterViewId), [object copy], OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (NSString *)tl_viewId
{
    return objc_getAssociatedObject(self, @selector(specterViewId));
}
#pragma clang diagnostic pop

- (NSString *)tl_controllerVariable
{
    NSString *result = nil;
    if ([self isKindOfClass:[UIControl class]]) {
...
static NSString *tl_encryptHelper(id input)
{
...
- (NSString *)tl_varA
{
    return tl_encryptHelper([self tl_viewId]);
}

- (NSString *)tl_varB
{
    return tl_encryptHelper([self tl_controllerVariable]);
}

- (NSString *)tl_varC
{
    return tl_encryptHelper([self tl_imageFingerprint]);
}

- (NSArray *)tl_varSetD
{
    NSArray *targetActions = [self tl_targetActions];
    NSMutableArray *encryptedActions = [NSMutableArray array];
    for (id targetAction in targetActions) {
        [encryptedActions addObject:tl_encryptHelper(targetAction)];
    }
...
- (NSString *)tl_varE
{
    return tl_encryptHelper([self tl_text]);
}
